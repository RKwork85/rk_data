...
我们计划将VideoCombinator改造为基于FastAPI + Redis + RQ的异步任务处理系统。以下是改造步骤：1. 将视频生成任务封装为独立函数，以便由RQ Worker执行。2. 创建FastAPI应用，提供任务提交、状态查询等接口。3. 使用Redis作为消息队列，RQ作为任务队列管理。4. 编写RQ Worker脚本来处理任务。由于原始代码中`VideoCombinator`类包含状态（如素材使用计数器、生成记录等），在分布式环境中需要重新设计： - 状态不能保存在内存中（因为多个Worker实例需要共享状态） - 需要将状态存储在Redis中（例如使用Redis的哈希表存储每个素材的使用次数，集合存储组合历史等）但考虑到重构工作量巨大且原代码逻辑复杂，我们可以采取另一种方式：将每个视频生成任务视为独立任务，每次生成一个视频组合（而不是批量生成多个视频）。这样，每个任务都是无状态的，只需要知道输入参数和输出路径。然而，原代码中的`generate_and_render`方法会生成多个视频（最多`max_videos`个），并且有全局的素材使用限制（每个素材最多使用`max_usage`次）。如果拆分成多个独立任务，我们需要在Redis中维护全局的素材使用计数，并且要保证并发安全（多个Worker同时更新计数）。权衡之后，我们决定： - 每次提交一个视频生成任务（生成一个视频），这样任务之间是独立的，但需要共享素材使用计数。 - 使用Redis的原子操作（如INCR）来维护每个素材的使用次数。具体改造计划：1. 创建FastAPI应用，提供以下端点： - POST /tasks: 提交一个视频生成任务，参数包括输入目录、输出目录、任务参数（min_duration, max_duration, max_clips_per_video, max_usage等）以及需要生成的视频数量（可选，默认为1）。 - GET /tasks/{task_id}: 查询任务状态 - GET /tasks: 列出所有任务2. 任务提交后，将任务放入Redis队列，由Worker处理。3. 任务执行逻辑（Worker端）： - 每个任务生成一个视频。 - 使用Redis记录每个素材的使用次数（键如：`clip_usage:<clip_path>`）。 - 为了避免重复组合，我们可以使用Redis集合记录已经使用过的组合（组合指纹：排序后的素材路径元组）。4. 由于原代码中的`VideoCombinator`类设计为批量生成多个视频，我们需要将其拆分为每次生成一个视频。同时，将素材使用计数和组合历史存储在Redis中。5. 状态存储设计： - 素材使用次数：使用Redis哈希表，键为`clip_usage:<project_id>`（如果支持多项目），或者简单键`clip_usage`（单项目）。但为了支持多任务并发，我们使用Redis的原子操作INCR和DECR。 - 组合历史：使用Redis集合，键为`combination_history:<project_id>`，存储组合指纹（排序的素材路径元组的字符串表示）。6. 考虑到原代码中生成多个视频时是顺序生成（一个组合接一个组合），现在改为每个任务生成一个视频，因此需要将原代码中生成一个组合的逻辑提取出来，并修改为使用Redis进行状态管理。

我现在需要你将所有的代码按照这个思路写下来，以下是VideoCombinator的完整程序,注意要包含该程序的完整功能
'''
* @Description: 版本优化：修复缺失pdf字体及编码问题 + 自定义pdf的图表需求
* @Author: rkwork
* @Date: 2025-08-13
...
def setup_logger(output_folder):
    """配置并返回日志记录器"""
    log_file = os.path.join(output_folder, "video_combinator.log")
    ensure_dir(log_file)
    
    # 创建日志记录器
    logger = logging.getLogger("VideoCombinator")
...
class VideoCombinator:
    def __init__(self, input_folder, output_folder, min_duration=60, max_duration=300, 
                 max_clips_per_video=5, max_videos=50, max_usage=3, logger=None):
        """
        初始化视频组合器
    ...
    def analyze_and_visualize_clips(self, chinese_font_path=None):
        """分析并可视化素材使用情况和时长分布"""
        if not self.clip_records:
    ...
    def generate_and_render(self):
        ...
        # 分析素材使用情况和时长分布
        analysis_report_path = self.analyze_and_visualize_clips(ttf_path)
...
if __name__ == "__main__":
    ...
    output_folder = "api/video_mixer/deepseek4-4_output_videos"
    ttf_path = "api/video_mixer/assets/fonts/汉仪晓波花月圆W.ttf"
    ...
    combinator = VideoCombinator(
        ...
        min_duration=20,      # 25秒最小时长
        max_duration=45,      # 45秒最大时长
        max_clips_per_video=15, # 每个视频最多3个素材
        max_videos=100,         # 生成5个视频
        max_usage=15,          # 每个素材最多使用3次
        logger=logger
    )
    ...
    if video_records:
        ...
        for record in video_records:
            status = "成功" if record['渲染状态'] == '成功' else "失败"
            logger.info(f"  {status} {record['视频ID']}: {record['视频时长(秒)']:.1f}s, "
                      f"素材: {record['素材数量']}个, 耗时: {record['渲染耗时(秒)']:.2f}s")
...
│   │   ├── v1/
│   │   │   ├── endpoints/
│   │   │   │   ├── video_combinator.py   # 改造：任务提交接口
│   │   │   │   └── tasks.py              # 新增：任务查询接口 ✅
│   │
│   ├── core/
│   │   ├── video_combinator/
│   │   │   ├── combinator.py             # 改造：生成单视频逻辑 ✅
│   │   │   ├── redis_state_manager.py    # 新增：Redis状态管理 ✅
│   │   │   ├── task_runner.py            # 新增：任务执行入口 ✅
│   │
│   ├── models/
│   │   ├── schemas.py                    # 新增：任务Pydantic模型
│   │   ├── tasks.py                      # 改造：任务数据模型 ✅
│   │
│   ├── services/
│   │   ├── task_service.py               # 改造：任务管理服务 ✅
│   │   ├── redis_service.py              # 新增：Redis连接服务 ✅
│   │
│   ├── tasks/                            # 新增：任务定义目录 ✅
│   │   ├── video_generation.py           # 任务函数定义
│   │   └── __init__.py
│   │
│   ├── utils/
│   │   ├── redis_utils.py                # 新增：Redis工具 ✅
│   │
│   ├── worker.py                         # 新增：RQ Worker入口 ✅
│   ├── rq.py                             # 新增：RQ配置
│
├── scripts/
│   ├── rq_worker.sh                     # 新增：Worker启动脚本
│
├── requirements.txt                     # 新增：rq, redis
```
```